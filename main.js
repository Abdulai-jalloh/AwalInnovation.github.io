...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayLands (landsToDisplay) {
      ...
      landsToDisplay.forEach(land => {
        container.innerHTML += `<div class="land-card"> 
                                ...
                                <p>Prix:${land.price}</p>
                                <a href="Land-details.html?id=${land.id}">Voir plus</a>
                                </div> `;                     
      });
    }
  ...
  if (isDetails) {
    ...
    fetch('data/land.json').then(res => res.json())
    .then(lands => {
       allLands = lands;
      ...
      if (land) {
       ...
       container.innerHTML = ` <section class="gallery">${gallery}</section>
                               <h2>${land.title}</h2>
  ...
  // console.log(displayLands)
    function displayLands(landsToDisplay) {
      ...
      landsToDisplay.forEach(land =>{
        ...
        <p> <strong> Emplacement: </strong>${land.location} </p> 
        <p> <strong> Prix: </strong>${land.price} </p> 
        <a href="Land-details.html?id=${land.id}" class="view-more-btn">Voir plus</a> </section>`;
